...
<template>
  ...
  >
    ...
    <div class="relative">
      ...
      <div
        ref="lineRef"
        class="absolute left-1/2 -translate-x-1/2 top-0 bottom-0 w-px bg-gradient-to-b from-[var(--color-accent-start)] via-[var(--color-accent-end)] to-transparent origin-top md:block hidden"
      />
      <!-- Mobile line -->
      <div class="absolute left-6 top-0 bottom-0 w-px bg-gradient-to-b from-[var(--color-accent-start)] via-[var(--color-accent-end)] to-transparent md:hidden" />
      ...
      <div class="space-y-16">
        ...
        >
          ...
          <div class="hidden md:grid md:grid-cols-2 md:gap-8 items-start">
            ...
            <div :class="i % 2 === 0 ? 'text-right pr-12' : 'col-start-2 pl-12'">
              <div class="glass-card p-6 inline-block text-left">
                <div class="flex items-center gap-2 mb-2">
                  <div class="w-3 h-3 rounded-full bg-gradient-to-r from-[var(--color-accent-start)] to-[var(--color-accent-end)]" />
                  <span class="text-xs font-mono text-[var(--color-text-muted)]">{{ exp.period }}</span>
                </div>
                <h3 class="text-xl font-heading font-bold text-[var(--color-text-primary)]">
                  {{ exp.role }}
                </h3>
                <p class="text-[var(--color-accent-start)] font-medium">
                  ...
                  <span class="text-[var(--color-text-muted)]"> • {{ exp.location }}</span>
                </p>
            ...
            <!-- Node on the line -->
            <div class="absolute left-1/2 -translate-x-1/2 top-6 w-4 h-4 rounded-full border-2 border-[var(--color-accent-start)] bg-[var(--color-bg-base)]" />
          </div>

          <!-- Mobile: Stacked layout -->
          <div class="md:hidden pl-14 relative">
            <!-- Node -->
            <div class="absolute left-4 top-6 w-4 h-4 rounded-full border-2 border-[var(--color-accent-start)] bg-[var(--color-bg-base)]" />

            <div class="glass-card p-5">
              <div class="flex items-center gap-2 mb-2">
                <span class="text-xs font-mono text-[var(--color-text-muted)]">{{ exp.period }}</span>
              </div>
              <h3 class="text-lg font-heading font-bold text-[var(--color-text-primary)]">{{ exp.role }}</h3>
