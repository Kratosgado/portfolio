...
const filteredProjects = computed(() => {
  ...
  if (activeCategory.value !== 'all') {
    result = result.filter((p) => {
      ...
      if (cat === 'neovim')
        return type.includes('neovim') || stack.some((s: string) => s.includes('neovim') || s.includes('lua'));
      if (cat === 'web')
        return (
          type.includes('web') ||
          stack.some(
            (s: string) => s.includes('vue') || s.includes('nuxt') || s.includes('react') || s.includes('angular'),
          )
        );
      if (cat === 'mobile')
        return type.includes('mobile') || stack.some((s: string) => s.includes('flutter') || s.includes('kotlin'));
...
const featuredProjects = computed(() => filteredProjects.value.filter((p) => featuredSlugs.includes(p.slug ?? '')));

const regularProjects = computed(() => filteredProjects.value.filter((p) => !featuredSlugs.includes(p.slug ?? '')));
  ...
  <section id="projects" class="section-viewport max-w-7xl mx-auto">
    ...
    <div class="mb-12 text-center">
      ...
      <p class="section-subtitle mx-auto">
        Tools, apps, and systems I've built.
      </p>
    </div>
    ...
    <div v-if="pending" class="animate-pulse space-y-6">
      <div class="flex justify-center gap-2 mb-10">
        <div v-for="i in 6" :key="i" class="h-8 bg-white/5 rounded-full w-28" />
      </div>
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div v-for="i in 2" :key="i" class="glass-card p-6 space-y-3">
          <div class="h-4 bg-white/5 rounded w-1/4" />
          <div class="h-6 bg-white/5 rounded w-3/4" />
          <div class="h-4 bg-white/5 rounded w-full" />
          <div class="flex gap-2">
            <div
              v-for="j in 4"
              :key="j"
              class="h-5 bg-white/5 rounded-full w-16"
            />
          </div>
        </div>
      </div>
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        <div v-for="i in 6" :key="i" class="glass-card p-5 space-y-3">
          <div class="h-5 bg-white/5 rounded w-2/3" />
          <div class="h-4 bg-white/5 rounded w-full" />
          <div class="flex gap-1">
            <div
              v-for="j in 3"
              :key="j"
              class="h-5 bg-white/5 rounded-full w-14"
            />
          </div>
        </div>
      </div>
    ...
    <template v-else>
      ...
      >
        ...
        <div class="relative">
          <UIcon
            ...
            class="absolute left-3 top-1/2 -translate-y-1/2 size-4 text-[var(--color-text-muted)]"
          />
          <input
            v-model="searchQuery"
            type="text"
      ...
      >
        ...
        >
          <div class="flex items-center justify-between mb-3">
            ...
            <div class="flex items-center gap-2">
              ...
              >
                ...
                {{
                  project.installs >= 1000
                    ? `${+(project.installs / 1000).toFixed(1)}k`
                    : project.installs
                }}
              </span>
            </div>
          ...
          <h3
            class="text-xl font-heading font-bold text-[var(--color-text-primary)] group-hover:text-[var(--color-accent-start)] transition-colors"
          >
            {{ project.title }}
          </h3>
          <p
            class="text-sm text-[var(--color-text-secondary)] mt-2 line-clamp-2"
          >
            {{ project.description }}
          </p>
          ...
          <div
            class="flex items-center gap-3 mt-4 pt-3 border-t border-white/5"
          >
            <span
              class="text-xs text-[var(--color-text-muted)] group-hover:text-[var(--color-accent-start)] transition-colors flex items-center gap-1"
            >
      ...
      <div v-if="hasMore" class="flex justify-center mt-10">
        <button class="btn-neon flex items-center gap-2" @click="loadMore">
